...
router.get('/', async (req, res) => {  
    ...
    try { 
        if (name) {
           ...
           var apiresult = await axios.get(`https://api.rawg.io/api/games?search=${sname}&key=015df354c078483583181e1a27fc8c35`)
           apiresult = apiresult.data.results
        } else {
            async function SearchApi () {
              try { 
                 const promise1 = axios.get(`https://api.rawg.io/api/games?key=015df354c078483583181e1a27fc8c35`);
                 const promise2 = axios.get(`https://api.rawg.io/api/games?key=015df354c078483583181e1a27fc8c35`);
                 const promise3 = axios.get(`https://api.rawg.io/api/games?key=015df354c078483583181e1a27fc8c35`);
...
//Search a videogame by id
  router.get('/:id', async (req, res) => {  
    ...
    //Search videogame in the Api
         ...
         const result = await axios.get(`https://api.rawg.io/api/games/${idkey}?key=015df354c078483583181e1a27fc8c35 `)
         if (result.data.id) {
            let genrestr=[]
